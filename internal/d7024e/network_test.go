...
func TestGetLocalIP(t *testing.T) {
	...
	assert.Equal(t, GetLocalIP(), "130.240.110.63")
}
...
func TestStoreRPC(t *testing.T) {
	...
	storeRPC := NewRPC(con1, "localhost","STORE", "hej")
	go network.ListenHandler()
	go network.storeRPC(storeRPC)
	msg := <-network.external
	fmt.Printf("msg:" + string(msg))
	network.internal <- ([]byte(`{"Sender":{"ID":[180,51,128,91,44,141,121,44,117,229,176,105,242,23,223,203,192,244,81,122],"Address":"localhost","KeyValueStore":{}},"TargetAddress":"localhost","MessageType":"STORE","Content":"hej"}`))
